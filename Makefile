...
SYNC_DIR=/vagrant/
TEST_BOX_NAME=test/TCPPrague
LIBVIRT_VOL_NAME=test-VAGRANTSLASH-tcp_prague_box_image_0.img
...
%.test: bento/builds/$(DISTRIBUTION).%.box clean_test
	# TODO vagrant box add $< --name $(TEST_BOX_NAME) --provider $(basename $@)
	# TODO cd test && vagrant up --provider $(basename $@)
	# TODO cd test && vagrant ssh -- "sudo python $(SYNC_DIR)/run_tests.py"
	$(MAKE) clean_test
